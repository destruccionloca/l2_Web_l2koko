...
<div class="content">
    ...
    -->
        ...
        <div class="block-content">
            {!! Form::open(["url" => route('rate.sort'), 'method' => "post", "onsubmit" => "return setOrder();"]) !!}
                <div class="row js-draggable-items">
                    <ul class="draggable-column">
                        @foreach($rates as $rate)
                            <li class="block draggable-item rate" data-id="{{$rate->id}}">
                                <div class="block-header block-header-default">
                                    <h3 class="block-title">{{$rate->name}}</h3>
                                    <div class="block-options">
                                        <a class="btn-block-option draggable-handler" href="javascript:void(0)">
                                            <i class="si si-cursor-move"></i>
                                        </a>
                                    </div>
                                    <a type="button" class="btn btn-sm btn-secondary" href="{{route("rate.delete", ["rate" => $rate->id])}}" data-toggle="tooltip" title="Удалить">
        ...
        <div class="modal-content">
            <div class="block block-themed block-transparent mb-0">
                <div class="block-header bg-primary-dark">
                    <h3 class="block-title">Добавить Рейт</h3>
                    <div class="block-options">
                ...
                <div class="block-content">
                    {!! Form::open(["url" => route('rate.store'), 'method' => "post"]) !!}
                    <div class="form-group row">
                        <div class="col-md-12">
