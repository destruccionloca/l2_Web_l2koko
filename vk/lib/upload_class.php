...
class Upload {
    ...
    public function UploadPostImg () {
        $ROOT = $_SERVER['DOCUMENT_ROOT'];
            $storeFolder = $ROOT . '/uploads/images/main';   //2
            $post_id = (isset($_REQUEST['postidmain']))?$_REQUEST['postidmain']:$this->getId();
            $uploadDir = $storeFolder."/";
        if (!file_exists($uploadDir)) {
            mkdir($uploadDir);
        }
        if (!empty($_FILES)) {
            ...
            $filename = $post_id . $img_type;
            $src = $uploadDir . "/" . $filename;
            $tempFile = $_FILES['file']['tmp_name'];
            move_uploaded_file($tempFile,$src); //6
            /* Вызываем функцию с целью уменьшить изображение до ширины в 100 пикселей, а высоту уменьшив пропорционально, чтобы не искажать изображение */
            $this->resize($src, 480, 360); // Вызываем функцию
        }  else {
            $result  = array();
            $files = scandir($uploadDir);                 //1
            if ( false!==$files ) {
                foreach ( $files as $file ) {
                    if ( '.'!=$file && '..'!=$file && preg_match("/^".$post_id."\..*/", $file)) {       //2
                        $obj['name'] = $file;
                        $obj['size'] = filesize($uploadDir."/".$file);
                        $result[] = $obj;
                    }
